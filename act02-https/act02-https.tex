...
\textbf{Ligne 14 et 15}, il y a la déclaration du certificats et des clefs privée du serveur 
\begin{lstlisting}[style=commande]
...
\itemE Choisir le type de certificat
	\begin{itemize}
	\item[$\Rightarrow$] Certificat DV (Domain Validation) : Valide uniquement la propriété du domaine. 
	\item[$\Rightarrow$] Certificat OV (Organization Validation) : valide le nom de domaine mais aussi l'organisation/l'entreprise. Nécessite d'avoir des documents commerciaux
	\item[$\Rightarrow$] Certificat EV (Extended Validation) : Validation complète (existance commerciale, légal et physique). S'affiche avec une bande verte dans certains navigateurs
	\item[$\Rightarrow$] Certificat Wilcard : Sécurisé un nombre illimité de sous-domaine
	\end{itemize}
\end{itemize}
...
\begin{itemize}
  \itemE \href{https://ciel-broceliande.myasustor.com}{Enquête satisfaction académique}
\end{itemize}
...
\paragraph{Remarque préliminaire} : Ce dépôt contient des programmes client pour l’ESP32 ainsi que des programmes client en Python destinés à des machines standards. Ce code pourra être d'avantage exploité dans une séquence sur la sécurité des IoT plus particulièrement celle du Wifi.  
