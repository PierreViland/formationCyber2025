...
\paragraph{Solutions alternatives : Zabbix}. C'est  une autre solution de surveillance open-source utilisée pour collecter des métriques, surveiller des applications et des infrastructures réseau en temps réel, ainsi que pour envoyer des alertes en cas de problèmes.
\end{minipage}
...
Comparé à Prometheus, Zabbix a une approche plus centralisée et traditionnelle ce qui le rend moins performant dans des environnements de \textbf{microservices} et de conteneurs. Il sera étudié dans une prochaine activité.
...
Durant cette activité, vous allez utiliser trois dépôts : 
...
\end{itemize}
\href{https://github.com/PierreViland/00-serveurLemp.git}{git@github.com:PierreViland/00-serveurLemp.git}
\end{minipage}
\begin{minipage}{0.39\linewidth}
\begin{center}
...
Dans un premier temps, vous allez exploiter \textbf{le serveur maison Iot}. L'objectif est de récupérer des informations sur l'état du serveur, telles que le nombre de connexions ou le temps moyen entre chaque connexion.

En résumé, le système mis en place est le suivant
...
\vspace{0.5cm}
Si ce n'est pas déjà fait, vous pouvez récupérer le client et le serveur python (comme dans l'activité précédente) dans le dépôt  
\begin{itemize}
...
\vspace{0.5cm}
Vous pouvez soit utiliser la version HTTP (répertoire \verb?03-base_http+Monitoring? soit la version HTTPS (répertoire \verb?04-https+monitoring?). Par soucis de simplicité, la version HTTP est testée dans cet exemple. Dans tous les cas, les deux fichiers à exploiter sont : 
...
\begin{itemize}
\itemE Un exportateur est créé via un serveur HTTP en début de fichier
\end{itemize}
...
Pour lancer correctement la monitoring stack, il est nécessaire de regarder si le fichier  de configuration de Prometheus nommé \verb?prometheus.yml? est correct :
...
Si tout s'est bien déroulé, vous devez avoir une fenêtre similaire à celle ci-dessous.
...
\subsection{Grafana}
Vous avez vu précédemment que \textbf{Prometheus} était adapté pour récupérer toutes sortes de métrique. \textbf{Grafana} va permettre d'afficher des  tableaux de bord ("dashbord") pour afficher sous forme de graphique des rapports de monitoring. 
  ...
  alertmanagers:
    - static_configs:
        - targets:
            - 'alertmanager:9093'
\end{lstlisting}
Ces lignes configurent Prometheus pour qu’il envoie des alertes à un serveur "Alertmanager" qui écoute sur le port 9093 dès qu'une règle d'alerte est déclenchée.
